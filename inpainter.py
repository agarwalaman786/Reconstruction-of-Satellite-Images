...
class Inpainter():
    ...
    def __init__(self, image, mask,filename, patch_size=9, plot_progress=True):
    ...
    def _plot_image(self):
        ...
        if self.cnt==1:
            os.mkdir(path)
            imsave(path+'t'+str(self.cnt)+'.jpg',image,quality=100)
            #plt.pause(1)
            ls = os.listdir(path)
            ls1 = sorted(ls,key = lambda x: int(x.split('t')[1].split('.')[0]))
            ls2 = []
            for i in ls1:
                img = Image.open(path+i)
                ls2.append(np.array(img))
            imageio.mimsave(os.path.join(os.path.dirname(os.path.abspath(__file__))) + '/Gif/' + self.filename.split('.')[0] + '.gif',ls2)
        else:
            imsave(path+'t'+str(self.cnt)+'.jpg',image,quality=100)
            #plt.pause(1)
            ls = os.listdir(path)
        ...
        #imsave(os.path.join(os.path.dirname(os.path.abspath(__file__)) + '/temp/','t'+str(self.cnt)+'.jpg'), image, quality=100)
        #plt.draw()
    ...
    def _finished(self):
        ...
        if self.compvar==(total-remaining):
            return True
        else:
            self.compvar = total-remaining
